...
with plan_tab:
    ...
    if (
        ...
        columns_to_format = df_to_show.columns[[2, 3, 4, 5]]

        # 2. Buat dictionary untuk konfigurasi kolom
        currency_format_config = {
            col: st.column_config.NumberColumn(
                label=col,  # Anda bisa ganti label jika mau, misal: "Harga Jual (Rp)"
                format="accounting",
            )
        ...
        st.dataframe(
            ...
            width="stretch",
            hide_index=True,
            column_config=currency_format_config,  # Tambahkan parameter ini
        )
...
with budget_ads_tab:
    ...
    with mp_tab:
        ...
        for i, project_name in enumerate(tab_names):
            with tabs[i]:
                ...
                preview_key = (
                    f"show_preview_{project_name.lower().replace(' ', '_')}_ads"
                )
                ...
                with st.form(f"form_{project_name.lower().replace(' ', '_')}_ads"):
                    ...
                    st.write(
                        "Tekan tombol di bawah untuk pratinjau dan menyimpan data."
                    )
                    submitted = st.form_submit_button("Simpan & Pratinjau")
                ...
                if st.session_state.get(preview_key, False):
                    ...
                    if not cleaned_df.empty:
                        ...
                        st.write(
                            "Silakan cek kembali data Anda sebelum disimpan permanen."
                        )
    ...
    with reg_tab:
        cpas_tab, ctwa_tab, fb_ads_tab = st.tabs(["CPAS", "FB CTWA", "FB ADS"])
        with cpas_tab:
            st.title("ini tab cpas")
        with ctwa_tab:
            st.title("ini tab fb ctwa")
        with fb_ads_tab:
            st.title("ini tab fb_ads")
